...
struct AgentAvatarView: View, Equatable {
    ...
    /// Idle bob (kept from v1) — only when standing and not actively walking.
    ...
    /// Fast vertical jiggle that fakes a walk cycle when the ghost is moving.
    /// Only active when actively walking and not at-desk.
    private func computeWalkJiggle(at elapsed: TimeInterval) -> CGFloat {
        guard snapshot.isWalking, !snapshot.isAtDesk else { return 0 }
        let frequency: Double = 4 // Hz
