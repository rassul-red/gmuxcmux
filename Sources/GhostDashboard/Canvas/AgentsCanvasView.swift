...
/// Top-level "game" canvas shown in the middle column of cmux. Renders one
/// room per workspace as a horizontal row inside a scrollable, pinch-zoomable
/// world. Agents walk around their rooms when not busy and park at chairs
/// when running/asking.
///
/// Snapshot-boundary rule: row subviews see only immutable value snapshots
/// + closures. World position writes happen exclusively in
...
struct AgentsCanvasView: View {
    ...
    @StateObject private var roleStore = AgentRoleStore.shared
            ...
            HStack(alignment: .top, spacing: roomGap) {
                ForEach(rooms) { room in
                    AgentsCanvasRoomView(
                        ...
                        onTapAgent: { workspaceId, panelId in
